...
def generate_query_file(folder: str, oid: str, query: ObjectQuery):
    ...
    with open(filename, "w", encoding="utf-8") as f:
        ...
        f.write(f"{str(query.state)}\n")
        for key, value in query.properties.items():
            f.write(f"{key}={value}\n")

def generate_input_data(folder: str, frames: List[Frame], queries: Dict[str, ObjectQuery]):
    ...
    for oid, query in queries.items():
        generate_query_file(folder, oid, query)
...
class TrackerFolderRuntime(TrackerRuntime):
    ...
    * Additional lines contain optional arguments in the form of key=value
    ...
    def __init__(self, tracker: Tracker, command: str, log: bool = False, timeout: int = 30, linkpaths=None, envvars=None, arguments=None):
        ...
        Arguments:
            ...
            arguments {dict} -- The additional arguments to pass to the tracker process.
        """
    ...
    def run(self, frames: List[Frame], queries: Queries) -> RunStatus:
        ...
        generate_input_data(self.folder, frames, queries)
