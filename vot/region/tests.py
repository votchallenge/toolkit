...
class TestRasterMethods(unittest.TestCase):
    ...
    def test_rasterize_polygon(self):
        ...
        np.testing.assert_array_equal(rasterize_polygon(points, (0, 0, 100, 100)), np.ones((100, 100), dtype=np.uint8))

    def test_rasterize_rectangle(self):
        np.testing.assert_array_equal(rasterize_rectangle(np.array([[0], [0], [100], [100]], dtype=np.float32), (0, 0, 100, 100)), np.ones((100, 100), dtype=np.uint8))

    def test_copy_mask(self):
        mask = np.ones((100, 100), dtype=np.uint8)
        np.testing.assert_array_equal(copy_mask(mask, (0, 0), (0, 0, 100, 100)), np.ones((100, 100), dtype=np.uint8))
